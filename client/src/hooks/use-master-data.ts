...
export function useMasterData(activeTab: string) {
    ...
    const bookingsQuery = useQuery<EnrichedBooking[]>({
        ...
        enabled: !!user && (activeTab === "bookings" || activeTab === "calendar" || activeTab === "overview"),
        staleTime: 30000,
    ...
    return {
        ...
        portfolioQuery,
        notificationsQuery,
        isLoading: masterQuery.isLoading || (activeTab === "analytics" && statsQuery.isLoading) || (activeTab === "schedule" && scheduleQuery.isLoading),
    };
}
