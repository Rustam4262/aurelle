    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle className="font-serif text-2xl">{t("marketplace.salon.bookService")}</DialogTitle>
                </DialogHeader>
                    ...
                    {selectedService && (
                        <div className="p-4 bg-primary/5 rounded-xl border border-primary/10">
                            ...
                            <p className="text-sm text-muted-foreground flex items-center gap-2 mt-1">
                                <span>{selectedService.duration} {t("marketplace.salon.minutes")}</span>
                                <span>•</span>
                                <span className="font-semibold text-primary">{formatCurrency(selectedService.priceMin)}</span>
                            </p>
                        </div>
                    )}
                    {selectedMaster && (
                ...
                <DialogFooter className="flex-col sm:flex-row gap-3 pt-4 border-t">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                        ...
                        >
                            {isSubmitting
                                ? t("marketplace.salon.booking")
                                : t("marketplace.salon.confirm")}
                        </Button>
                    </div>
                </DialogFooter>
            </DialogContent>
