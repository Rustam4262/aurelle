        ...
        <Card className="p-4 transition-all hover:shadow-md" data-testid={`card-master-${master.id}`}>
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {specs.length > 0 && <p className="text-primary text-sm mb-1 truncate">{specs.join(", ")}</p>}
                    {master.experience && (
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {masters && masters.length > 0 ? (
                ...
                <Card className="p-8 text-center bg-muted/30">
                    <p className="text-muted-foreground">{t("marketplace.salon.noMasters") || "No team members listed"}</p>
                </Card>
            )}
        </div>
    );
