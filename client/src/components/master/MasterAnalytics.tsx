...
export function MasterAnalytics({ stats, isLoading }: MasterAnalyticsProps) {
    ...
    if (!stats) {
        return <p className="text-center text-muted-foreground py-8">{t("marketplace.master.noData")}</p>;
    }
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card className="p-6">
                    ...
                    <p className="text-3xl font-bold text-foreground">{stats.totalEarnings.toLocaleString()} UZS</p>
                </Card>
                <Card className="p-6">
                    <h3 className="font-medium text-foreground mb-2">{t("marketplace.master.clientRetention")}</h3>
                    <p className="text-3xl font-bold text-foreground">{(stats.clientRetention.retentionRate * 100).toFixed(0)}%</p>
                    <p className="text-sm text-muted-foreground">
                        {stats.clientRetention.repeatClients} / {stats.clientRetention.totalClients} {t("marketplace.master.repeatClients")}
                    </p>
                </Card>
            </div>
                ...
                {stats.earningsByMonth.length > 0 ? (
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={stats.earningsByMonth}>
                            ...
                            <XAxis dataKey="month" />
                            <YAxis />
                            <Tooltip formatter={(value: number) => [`${value.toLocaleString()} UZS`, t("marketplace.master.earnings")]} />
                    ...
                    {stats.popularServices.length > 0 ? (
                        <ResponsiveContainer width="100%" height={250}>
                            <BarChart data={stats.popularServices} layout="vertical">
                                ...
                                <YAxis
                                    type="category"
                                    dataKey={(entry) => getLocalizedText(entry.service?.name, currentLang) || entry.serviceId.slice(-6)}
                                    width={120}
                                />
                                <Tooltip />
