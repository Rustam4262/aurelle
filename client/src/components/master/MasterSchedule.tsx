...
export function MasterSchedule({ initialSchedule, onSave, isSaving, isLoading }: MasterScheduleProps) {
    const { t } = useTranslation();
    const [schedule, setSchedule] = useState<ScheduleDay[]>([]);

    useEffect(() => {
        if (initialSchedule && initialSchedule.length > 0) {
            setSchedule(initialSchedule.map(h => ({
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    {[1, 2, 3, 4, 5, 6, 0].map((dayOfWeek) => {
                        ...
                        return (
                            <div key={dayOfWeek} className="flex items-center gap-4 p-4 bg-muted/50 rounded-md flex-wrap">
