...
export function HomeNavigation({ scrolled }: { scrolled: boolean }) {
    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex items-center justify-between h-16">
                    <Link href="/" className="flex items-center gap-2" data-testid="link-logo">
                        <span
                            className={`font-serif text-2xl font-semibold tracking-tight ${scrolled ? "text-foreground" : "text-white"
                                }`}
                        >
                            AURELLE
                        </span>
                    ...
                    <div className="hidden md:flex items-center gap-6">
                        <Link href="/">
                            <span
                                className={`text-sm font-medium transition-colors hover:opacity-80 cursor-pointer ${scrolled ? "text-foreground" : "text-white/90"
                                    }`}
                            >
                    ...
                    <div className="hidden md:flex items-center gap-2">
                        ...
                        <Link href="/owner">
                            <Button
                                className={`rounded-full px-6 transition-all hover:scale-105 active:scale-95 ${scrolled
                                        ? "shadow-sm"
            ...
            {mobileMenuOpen && (
                <div id="mobile-menu" className="md:hidden bg-background border-t border-border animate-in slide-in-from-top duration-300">
                    <div className="px-6 py-6 flex flex-col gap-4">
                        ...
                        <div className="pt-4 border-t flex flex-col gap-3">
                            ...
                            <Link href="/owner" onClick={() => setMobileMenuOpen(false)}>
                                <Button className="rounded-full w-full">{t("marketplace.nav.registerSalon")}</Button>
                            </Link>
                        </div>
                    </div>
                </div>
