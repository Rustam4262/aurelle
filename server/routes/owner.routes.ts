...
router.patch("/salons/:id/status", isAuthenticated, async (req: any, res) => {
  try {
    ...
    if (!parsed.success) {
      return res
        .status(400)
        .json({ error: "Invalid status value. Must be draft, active, or paused." });
    }
  ...
  try {
    ...
    const allMasters = await db
      .select({
        ...
        name: masters.name,
        email: masters.email,
        specialization: masters.specialties,
        bio: masters.bio,
        isActive: masters.isActive,
        rating: masters.averageRating,
...
router.post("/masters/:masterId/portfolio", isAuthenticated, async (req: any, res) => {
  try {
    ...
    // Add image to portfolio table
    const [newPortfolioImage] = await db
...
router.post("/services/:serviceId/duplicate", isAuthenticated, async (req: any, res) => {
  try {
    ...
    // Get source service
    const [sourceService] = await db.select().from(services).where(eq(services.id, serviceId));
...
router.get("/services/:id/masters", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const assignments = await db
      .select({ masterId: masterServices.masterId })
      .from(masterServices)
...
router.patch(
  "/services/:id/masters",
  isAuthenticated,
  requirePermission(OWNER_PERMISSIONS.MANAGE_SERVICES),
  async (req: any, res) => {
    try {
      const ownerId = req.user.claims.sub;
      const { id: serviceId } = req.params;
      ...
      if (masterIds.length > 0) {
        const mastersToAssign = await db
          .select()
          .from(masters)
          .where(and(inArray(masters.id, masterIds), eq(masters.salonId, service.salonId)));
...
router.patch(
  ...
  async (req: any, res) => {
    try {
      ...
      await logAudit({
        actorId: ownerId,
        action: `service.${isActive ? "activate" : "deactivate"}`,
        entityType: "services",
        entityId: serviceId,
        salonId: service.salonId,
        details: {
...
router.post(
  ...
  async (req: any, res) => {
    try {
      ...
      const allServices = await db.select().from(services).where(inArray(services.id, serviceIds));
      ...
      const salonIds = Array.from(new Set(allServices.map((s) => s.salonId)));
      const ownerSalons = await db
        .select()
        .from(salons)
        .where(and(inArray(salons.id, salonIds), eq(salons.ownerId, ownerId)));
      ...
      await logAudit({
        actorId: ownerId,
        action: `service.bulk_${isActive ? "activate" : "deactivate"}`,
        entityType: "services",
        entityId: "",
        details: {
          serviceIds,
  ...
  async (req: any, res) => {
    try {
      ...
      const bookingsToUpdate = await db
        .select()
        .from(bookings)
        .where(and(inArray(bookings.id, bookingIds), inArray(bookings.salonId, salonIds)));
      ...
      const successfulUpdates = updates.filter((u): u is NonNullable<typeof u> => Boolean(u));
...
router.get("/dashboard/overview", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const todayBookings = await db
      .select()
      .from(bookings)
      .where(and(inArray(bookings.salonId, salonIds), eq(bookings.bookingDate, today)));
    ...
    const weekBookings = await db
      .select()
      .from(bookings)
      .where(and(inArray(bookings.salonId, salonIds), gte(bookings.bookingDate, twoWeeksAgo)))
      .orderBy(desc(bookings.bookingDate));

    const thisWeekBookings = weekBookings.filter((b) => new Date(b.bookingDate) >= weekAgo);
    ...
    // Month stats
    const monthBookings = weekBookings.filter((b) => new Date(b.bookingDate) >= monthStart);
...
router.get("/dashboard/alerts", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const pendingBookings = await db
      .select()
      .from(bookings)
      .where(and(inArray(bookings.salonId, salonIds), eq(bookings.status, "pending")));
...
router.get(
  ...
  isAuthenticated,
  requirePermission(OWNER_PERMISSIONS.READ_ANALYTICS),
  async (req: any, res) => {
    try {
      const ownerId = req.user.claims.sub;
  ...
  async (req: any, res) => {
    try {
      ...
      let [clientProfile] = await db
        .select()
        .from(userProfiles)
        .where(eq(userProfiles.phone, clientPhone));
      ...
      if (!clientProfile) {
        const [newProfile] = await db
          ...
          .values({
            ...
            phone: clientPhone,
            role: "client",
            userId: randomUUID(), // Generated ID for guest (no auth account)
          })
          .returning();
      ...
      if (masterId) {
        ...
        await createNewBookingNotification(db, masterId, bookingDateStr, startTime, booking.id);
      }
...
router.post(
  "/salons/:id/breaks",
  isAuthenticated,
  requirePermission(OWNER_PERMISSIONS.MANAGE_SALONS),
  async (req: any, res) => {
    try {
      const { id: salonId } = req.params;
      const ownerId = req.user.claims.sub;
...
router.get("/salons/:id/exceptions", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const conditions = [eq(salonExceptions.salonId, salonId)];

    // Optional date range filtering
    if (from) {
      conditions.push(gte(salonExceptions.exceptionDate, from as string));
    }
    if (to) {
      conditions.push(lte(salonExceptions.exceptionDate, to as string));
    }
    ...
    const exceptions = await db
      ...
      .where(and(...conditions))
      .orderBy(salonExceptions.exceptionDate);
  ...
  async (req: any, res) => {
    try {
      ...
      if (!parsed.success) {
        return res
          .status(400)
          .json({ error: "Invalid exception data", details: parsed.error.errors });
      }
...
router.patch(
  "/salons/:salonId/exceptions/:exceptionId",
  isAuthenticated,
  requirePermission(OWNER_PERMISSIONS.MANAGE_SALONS),
  async (req: any, res) => {
    try {
...
router.delete(
  ...
  async (req: any, res) => {
    try {
      ...
      await db
        .delete(salonExceptions)
        .where(and(eq(salonExceptions.id, exceptionId), eq(salonExceptions.salonId, salonId)));

      await logAudit({
        actorId: ownerId,
        action: "salon.delete_exception",
        entityType: "salons",
        entityId: salonId,
        salonId,
        details: { exceptionId },
  ...
  async (req: any, res) => {
    try {
      ...
      if (!parsed.success) {
        return res
          .status(400)
          .json({ error: "Invalid invitation data", details: parsed.error.errors });
      }
      ...
      const [existingManager] = await db
        .select()
        .from(salonManagers)
        .where(and(eq(salonManagers.salonId, salonId), eq(salonManagers.userId, user.id)));
      ...
      if (existingManager) {
        if (existingManager.status === "active") {
          ...
          const [updated] = await db
            ...
            .set({
              status: "pending",
              permissions: permissions || (DEFAULT_MANAGER_PERMISSIONS as unknown as string[]),
              invitedAt: new Date(),
              revokedAt: null,
              revokedBy: null,
            })
...
router.post(
  "/salons/:salonId/managers/:managerId/accept",
  isAuthenticated,
  async (req: any, res) => {
    try {
      ...
      await logAudit({
        actorId: userId,
        action: "salon.accept_manager_invitation",
        entityType: "salons",
        entityId: salonId,
        salonId,
        details: {
...
router.patch(
  "/salons/:salonId/managers/:managerId/permissions",
  isAuthenticated,
  requirePermission(OWNER_PERMISSIONS.MANAGE_SALONS),
  async (req: any, res) => {
    try {
      ...
      const [updated] = await db
        .update(salonManagers)
        .set({ permissions })
        .where(and(eq(salonManagers.id, managerId), eq(salonManagers.salonId, salonId)))
        .returning();
      ...
      await logAudit({
        actorId: ownerId,
        action: "salon.update_manager_permissions",
        entityType: "salons",
        entityId: salonId,
        salonId,
        details: {
...
router.delete(
  "/salons/:salonId/managers/:managerId",
  isAuthenticated,
  requirePermission(OWNER_PERMISSIONS.MANAGE_SALONS),
  async (req: any, res) => {
    try {
