...
export async function registerRoutes(
  ...
  // Helper function to get client profile - any authenticated user can be a client
  async function getClientFromUser(userId: string): Promise<{ error: string; status: number } | { profile: typeof userProfiles.$inferSelect | null; userId: string }> {
    ...
    // Any authenticated user can access client dashboard - no role check needed
  ...
  app.get("/api/client/bookings", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // If user has no profile, they have no bookings
      if (!result.profile) {
        return res.json([]);
      }

      // Use profile.id (not userId) since bookings.clientId references userProfiles.id
      ...
      // Batch loading: collect unique IDs first
      ...
      // Fetch all related data in parallel (3 queries instead of 3*N)
      // Guard against empty arrays to prevent invalid SQL IN () expressions
      const [salonsData, servicesData, mastersData] = await Promise.all([
        salonIds.length > 0 
          ? db.select().from(salons).where(sql`${salons.id} IN (${sql.join(salonIds.map(id => sql`${id}`), sql`, `)})`)
          : Promise.resolve([]),
        serviceIds.length > 0
          ? db.select().from(services).where(sql`${services.id} IN (${sql.join(serviceIds.map(id => sql`${id}`), sql`, `)})`)
          : Promise.resolve([]),
        masterIds.length > 0
          ? db.select().from(masters).where(sql`${masters.id} IN (${sql.join(masterIds.map(id => sql`${id}`), sql`, `)})`)
          : Promise.resolve([]),
      ]);
  ...
  app.delete("/api/client/bookings/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { id } = req.params;
      const result = await getClientFromUser(userId);
      ...
      // User must have a profile to have bookings
      if (!result.profile) {
        return res.status(404).json({ error: "Booking not found" });
      }

      // Verify the booking belongs to this client (using profile.id, not userId)
  ...
  // Get client favorites with salon info
  app.get("/api/client/favorites", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const result = await getClientFromUser(userId);
  ...
  // Remove favorite
  app.delete("/api/client/favorites/:salonId", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { salonId } = req.params;
      const result = await getClientFromUser(userId);
  ...
  // Get client reviews
  app.get("/api/client/reviews", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const result = await getClientFromUser(userId);
      ...
      // Get salon info for each review
  ...
  app.put("/api/client/reviews/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { id } = req.params;
      const result = await getClientFromUser(userId);
  ...
  app.delete("/api/client/reviews/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { id } = req.params;
      const result = await getClientFromUser(userId);
